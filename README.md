...
| PBAlign | Minimum coverage (min_coverage) | 5  | The minimum site coverage that must be achieved for variant calls and consensus to be calculated for a site. |
| PBAlign | Compute methyl fraction (compute_methyl_fraction) | FALSE  | When identifying specific modifications (m4C and/or m6A), enabling this option will estimate the methylated fraction, along with 95% confidence interval bounds. |
| PBAlign | Identify basemods (identify) |   | Specific modifications to identify (comma-separated list). Currrent options are m6A and/or m4C. |
| PBAlign | Max sequence length (max_length) | 2112827392  | Maximum number of bases to process per contig |
| PBAlign | P-value (pvalue) | 0.001  | P-value cutoff |
| PBAlign | Minimum methylated fraction (min_fraction) | 30  | Minimum methylated fraction |
| PBAlign | Minimum Qmod score (min_score) | 30  | Minimum Qmod score to use in motif finding |
...
| PBAlign | Batch sort size (batch_sort_size) | 10000  | Intermediate sort size parameter (default=10000) |
| PBAlign | Force the number of regions (force_num_regions) | FALSE  | If supplied, then try to use this number (max value = 40000) of regions per reference, otherwise the coverage summary report will optimize the number of regions in the case of many references. Not compatible with a fixed region size. |
| PBAlign | Number of variants (how_many) | 100  | number of top variants to show (default=100) |
| PBAlign | Max contigs (max_contigs) | 25  | Max number of contigs to plot. Defaults to 25. |
| PBAlign | Number of regions (num_regions) | 1000  | Desired number of genome regions in the summary statistics (used for guidance, not strict). Defaults to 1000 |
| PBAlign | Region size (region_size) | 0  | If supplied, used a fixed genomic region size |
